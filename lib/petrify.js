var fs = require('fs'),
    ...
    behaviours = require('behaviors'),
    jsontemplate = require('json-template'),
    child_process = require('child_process'),
    findit = require('findit'),
    wmd = require('wmd');

// allow exists to work in Node 0.8+ (fs) and prior (path)
...
exports.emit = function(output_dir, urlpath, data, callback){
    ...
    if(filename.substr(0, output_dir.length) == path.normalize(output_dir)){
        exists(dirname, function(fileExists){
            if(!fileExists){
...
exports.run = function(opt){
    ...
    async.auto({
        ...
        removedir: function(callback){
            exists(opt.output_dir, function(fileExists){
                if(fileExists){
