...
/**
 * return relative prefix url that would put you at the root, so you can be prepended to asset URLs
 * Assumes relFilename is relative filename not ending in slash, so simply counts slashes and 
 * calculates ../ for each one
 */   
function relativePathToTop(relFilename){
  var relFileSplitArray = relFilename.split('/');
  relFileSplitArray.pop(); //remove one
  return relFileSplitArray.map(function(f){ return '../'; }).join('');
}
	...
	article.escaped_html_no_heading = html_escape(article.html_no_heading);
      ...
      html = html.replace( /__BASE_PREFIX__/g , article.assets_url_prefix); //replace prefix in nav and other partials
